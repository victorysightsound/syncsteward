...
impl SyncStewardMcpServer {
    ...
    async fn run_cycle(
        &self,
        Parameters(request): Parameters<DryRunRequest>,
    ) -> McpResult<RunCycleReport> {
        let config_path = self.config_path.clone();
        let report = tokio::task::spawn_blocking(move || {
            core_run_cycle(config_path.as_deref(), request.dry_run)
        })
        .await
        .map_err(|error| error.to_string())?
        .map_err(|error| error.to_string())?;
    ...
    #[tool(
        description = "Run one daemon-ready scheduled tick that only executes the approved target cycle when it is due. Supports dry-run mode for safe validation."
    )]
    async fn runner_tick(
        &self,
        Parameters(request): Parameters<DryRunRequest>,
    ) -> McpResult<RunnerTickReport> {
        let config_path = self.config_path.clone();
        let report = tokio::task::spawn_blocking(move || {
